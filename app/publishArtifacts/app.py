...
def upload_results_bucket(test_result_file, download_path):
    ...
    for file in test_result_file:
        ...
        upload_path = '{}-{}'.format(
            download_path.replace('/tmp/', ''), file.name)
        s3_client.upload_file(test_result_file, ResultsBucket, upload_path)
        logger.info("%s uploaded to %s ...",
                    test_result_file, ResultsBucket)
...
def unpack_test_results(test_result_file, download_path):
    ...
    if test_result_file.endswith("tar.gz"):
        ...
        try:
            tar = tarfile.open(
                '{}-{}'.format(download_path, test_result_file), "r:gz")
            ...
            logger.info("unpacking %s failed. %s",
                        test_result_file, sys.exc_info()[0])
            return
